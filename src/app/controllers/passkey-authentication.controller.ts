...
export class PasskeyAuthenticationController {
    ...
    async loginStart(ctx: Context, {}, body: LoginStartRequest) {
        ...
        const options = await generateAuthenticationOptions({
            timeout: 60000,
            allowCredentials: await this.credentialService.getAllowedCredentials(user.id),
    ...
    async loginFinish(ctx: Context) {
        ...
        const credential = await this.credentialService.getById(user, credentialId);
        if (!credential) {
        ...
        if (verified) {
            ...
            console.log(`loginFinish for email ${user.email} with authenticationInfo ${JSON.stringify(authenticationInfo,null,2)}`);
